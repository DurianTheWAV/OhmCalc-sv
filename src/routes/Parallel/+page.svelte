...
<div class="fields">
    ...
    <button onclick={() => { totalValue = parallel_resistor(resistor_list.split(',')); }}>{m.calcul()}</button>
    <p>{m.total_resistance()}: {totalValue} Ω</p>
</div>
<div class="description">
    <p>{@html m.parallel_r()}</p>
    <p><strong>1 / R<sub>total</sub> = 1 / R<sub>1</sub> + 1 / R<sub>2</sub> + ... + 1 / R<sub>n</sub></strong></p>
    <p>{m.where()}</p>
    <ul>
